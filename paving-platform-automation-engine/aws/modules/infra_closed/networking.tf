# Ops Manager/Bosh Director Subnet
...
resource "aws_subnet" "closed_subnets" {
  count             = length(var.availability_zones)
  vpc_id            = aws_vpc.vpc.id
  cidr_block        = cidrsubnet(local.closed_cidr, 2, count.index)
  availability_zone = element(var.availability_zones, count.index)

  tags = merge(var.tags, map("Name", "${var.env_name}-control-plane-closed"))
}

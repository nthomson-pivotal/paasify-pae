...
locals {
  ops_man_subnet_index = 0
  ops_man_subnet_id    = element(module.infra.infrastructure_subnet_ids, local.ops_man_subnet_index)
...
module "ops_manager_config" {
  source = "../../modules/ops_manager_config_closed"
...
module "control_plane" {
  source                  = "../../modules/control_plane_closed"
  vpc_id                  = module.infra.vpc_id
  env_name                = var.env_name
  availability_zones      = var.availability_zones
  vpc_cidr                = var.vpc_cidr
  private_route_table_ids = module.infra.egress_only_route_table_ids
...
module "uaa_lb" {
  ...
  internal              = true
  env_name              = var.env_name
  public_subnet_ids     = module.infra.closed_subnet_ids
...
module "dns" {
  ...
  ops_manager_record   = [aws_instance.ops_manager.private_ip]
...
resource "aws_instance" "ops_manager" {
  ...
  vpc_security_group_ids = [module.infra.vms_security_group_id]
...
module "vpc_endpoint" {
  ...
  vpc_id                     = module.infra.vpc_id
  region                     = var.region
  infrastructure_subnets_ids = module.infra.infrastructure_subnet_ids
